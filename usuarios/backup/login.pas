...
begin
    ...
    Reset(tfuet);
       ...
       ConectorLogin.LibraryLocation:= ExtractFilePath (Application.ExeName)+'librerias\'+renglon;
       readln(tfuet,renglon);
       ConectorLogin.Hostname:=renglon;
       readln(tfuet,renglon);
       ConectorLogin.Port:= strtoint(renglon)+3049;
       readln(tfuet,renglon);
...
ConsultaLogin.SQL.Add('select * from segur_usuarios');
ConsultaLogin.open;
...
begin
    ...
    begin
       // showmessage(trim(ConsultaLogin.FieldByName('passbuscar').asstring) + ' <--->  ' + trim(crip.encrypt(pass.Text)));
          if trim(crip.encrypt(pass.Text)) = trim(ConsultaLogin.FieldByName('passbuscar').asstring) then
          begin
